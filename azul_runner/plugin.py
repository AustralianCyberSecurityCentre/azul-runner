...
class Plugin:
    ...
    def __init__(self, config: settings.Settings | dict | None = None) -> None:
        ...
        self.DESCRIPTION = desc if desc else ""  # ty: ignore[invalid-attribute-access]
        ...
        for c in reversed(type(self).__mro__):
            if hasattr(c, "FEATURES"):
                if not isinstance(c.FEATURES, Iterable):
                    raise TypeError(f"FEATURES of {c} is not an iterable")
                for f in c.FEATURES:
                    if not isinstance(f, Feature):
                        raise TypeError(f"FEATURES of {c} contains non-Feature {f}")
                    features[f.name] = f
        self.FEATURES = sorted(features.values())  # ty: ignore[invalid-attribute-access]

        # override security through config
        self.SECURITY = self.cfg.security_override or self.SECURITY  # ty: ignore[invalid-attribute-access]
        if self.SECURITY:
    ...
    def reset(self, job: Job):
        ...
        # hash -> binary, tags
        self.data: dict[str, typing.BinaryIO | bytes] = {}
    ...
    def get_data_event(self, data_hash: str | None):
        ...
        if data_hash not in self._event_map:
            if self._job.event.entity.sha256 is None:
                raise ValueError("job event entity sha256 is empty, cannot create event")
